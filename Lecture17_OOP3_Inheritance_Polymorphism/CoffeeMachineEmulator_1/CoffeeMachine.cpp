...
{
    for (DrinkProgram* program : m_recipes)
    {
        delete program; //dynamically allocated memory MUST be manually deleted
    }

    m_recipes.clear(); //Sanity check. Vector will be cleared anyway automatically
}

void CoffeeMachine::addRecipe(DrinkProgram* program)
{
    m_recipes.push_back(program);
}
